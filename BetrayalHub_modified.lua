...
-- Crystal toggle state
local jd_crystalMode = "off" -- "off" | "infinite" | "instant"
...
-- ============================================================
-- CRYSTAL TOGGLE SECTION
-- ============================================================
local sec_crystal = tabJaneDoe:Section({ Title = "Crystal", Opened = true })

sec_crystal:Toggle({
    Title = "Infinite Hold",
    Flag = "jdCrystalInfiniteHold",
    Default = false,
    Type = "Checkbox",
    Callback = function(on)
        if on then
            jd_crystalMode = "infinite"
            pcall(function()
                print("[*] Loading Jane Doe Infinite Hold...")
                local found = false

                for _, v in pairs(getgc(true)) do
                    if type(v) == "table" then
                        if rawget(v, "DisplayName") == "Jane Doe" and rawget(v, "Crystal") then
                            local crystal = rawget(v, "Crystal")
                            crystal.MinChargeTime = 0.25
                            crystal.MaxChargeTime = 2
                            crystal.MaximumHoldTime = 99999
                            crystal.HoldTime = 99999
                            crystal.MaxHoldTime = 99999
                            crystal.AutoFireTime = 99999
                            print("[✓] Crystal config modified: UI fills normally, hold forever!")
                            found = true
                            break
                        end
                    end
                ...
                if not found then
                    for _, v in pairs(getgc(true)) do
                        if type(v) == "table" and rawget(v, "Crystal") then
                            local crystal = rawget(v, "Crystal")
                            if crystal and rawget(crystal, "MaxChargeTime") then
                                crystal.MinChargeTime = 0.25
                                crystal.MaxChargeTime = 2
                                crystal.MaximumHoldTime = 99999
                                crystal.HoldTime = 99999
                                crystal.MaxHoldTime = 99999
                                crystal.AutoFireTime = 99999
                                print("[✓] Fallback crystal table modified!")
                                found = true
                                break
                            end
                        end
                ...
                if not found then
                    print("[!] Couldn't find Jane Doe Crystal config. Make sure you are spawned in as Jane Doe.")
                end
            end)
        else
            jd_crystalMode = "off"
...
sec_crystal:Toggle({
    Title = "Instant Crystal",
    Flag = "jdCrystalInstant",
    Default = false,
    Type = "Checkbox",
    Callback = function(on)
        if on then
            jd_crystalMode = "instant"
            pcall(function()
                print("[*] Loading Instant Crystal Fix...")
                local found = false

                local function applyInstant(crystal)
                    crystal.MinChargeTime = 0.001
                    crystal.MaxChargeTime = 0.001
                    crystal.MaximumHoldTime = 0.001
                    crystal.HoldTime = 0.001
                    crystal.MaxHoldTime = 0.001
                    crystal.AutoFireTime = 0.001
                end

                for _, v in pairs(getgc(true)) do
                    if type(v) == "table" then
                        if rawget(v, "DisplayName") == "Jane Doe" and rawget(v, "Crystal") then
                            applyInstant(rawget(v, "Crystal"))
                            print("[✓] Jane Doe Crystal config modified for instant release!")
                            found = true
                            break
                        end
                    end
                ...
                if not found then
                    for _, v in pairs(getgc(true)) do
                        if type(v) == "table" and rawget(v, "Crystal") then
                            local crystal = rawget(v, "Crystal")
                            if crystal then
                                applyInstant(crystal)
                                print("[✓] Fallback crystal table modified for instant release!")
                                found = true
                                break
                            end
                        end
...
local guestCfg = {
    ...
    -- FIXED: Added soundBlockEnabled flag
...
-- FIXED: g_attemptBlockForSound now properly checks soundBlockEnabled
...
-- FIXED: g_hookSound now properly checks soundBlockEnabled
...
-- ============================================================
-- REMOTE AUTO BLOCK
-- The Network module routes ALL traffic through one set of
-- remotes using ConnectionRegistry to compress names to IDs.
-- Packet structure on OnClientEvent: (connectionId, packedArgs)
--   connectionId → number (registry ID) OR string (raw name)
--   packedArgs   → table where each value is a raw buffer
-- We hook all 3 hookable client remotes:
--   RemoteEvent          (reliable attacks)
--   UnreliableRemoteEvent (possible fast attacks)
--   BindableEvent         (local server→script signals)
-- and decode connectionId via ConnectionRegistry reverse map.
-- ============================================================
guestCfg.remoteBlockEnabled = false

local REMOTE_BLOCK_ABILITIES = {
    Slash=true, Stab=true, Punch=true, Swing=true, Dagger=true,
    Behead=true, Axe=true, GashingWound=true, Nova=true,
    ["Carving Slash"]=true, Carving=true, INFERNALCRY=true,
    MassInfection=true, PlasmaBeam=true,
}

-- Build reverse registry: number ID → string name
-- The Network module does this same loop internally (v_u_13)
local _rbRegistry = nil
local function rbGetRegistry()
    if _rbRegistry then return _rbRegistry end
    local ok, reg = pcall(function()
        return require(svc.RS:WaitForChild("ConnectionRegistry", 5))
    end)
...
-- Decode connectionId: could be number (registry ID) or string
local function rbDecodeAction(connectionId)
    if typeof(connectionId) == "string" then
        return connectionId
    elseif typeof(connectionId) == "number" then
        local reg = rbGetRegistry()
        return reg[connectionId]
    end
    return nil
end

-- Same buffer-name extraction as hbReadName but self-contained
local function rbReadName(raw)
    if typeof(raw) == "buffer" then
        local s = buffer.tostring(raw)
        -- The buffer format is: [type_byte=3][u32 length][string bytes]
        -- Strip leading binary/control bytes to get the plain name
        local name = s:match("[%a][%w%s%-_]+") or s:match("^[%c%z%p]*(.+)$") or s
        name = (name:match("^(.-)%s*$") or name)
        return name ~= "" and name or nil
    end
    return typeof(raw) == "string" and raw or nil
end

-- Core block trigger — shared by all 3 remote handlers
local function rbTryBlock()
    if not guestCfg.remoteBlockEnabled then return end
    ...
    if myRoot then
        ...
        if kf then
            ...
            for _, killer in ipairs(kf:GetChildren()) do
                ...
                if khrp then
                    local dist = (khrp.Position - myRoot.Position).Magnitude
                    if dist <= guestCfg.detectionRange
                    and isFacing(myRoot, khrp, killer.Name) then
                        inRange = true
                        break
                    end
                end
...
-- Handler shared by all 3 remotes
local function rbOnPacket(connectionId, packedArgs)
    if not guestCfg.remoteBlockEnabled then return end

    -- Decode the connection name from number ID or raw string
    local action = rbDecodeAction(connectionId)
    if action ~= "UseActorAbility" then return end

    -- packedArgs is a table of raw buffers — [1] holds the ability name buffer
    if typeof(packedArgs) ~= "table" or not packedArgs[1] then return end
    local name = rbReadName(packedArgs[1])
    if not name or not REMOTE_BLOCK_ABILITIES[name] then return end

    rbTryBlock()
end

local function setupRemoteBlock()
    -- Disconnect all existing hooks
    for _, c in ipairs(remoteBlockConns) do
    ...
    local netPath = svc.RS:FindFirstChild("Modules")
        and svc.RS.Modules:FindFirstChild("Network")
        and svc.RS.Modules.Network:FindFirstChild("Network")
    if not netPath then
        warn("[RemoteBlock] Network module path not found")
        return
    end

    -- 1. RemoteEvent — reliable, most attacks come through here
    local re = netPath:FindFirstChild("RemoteEvent")
    if re then
        table.insert(remoteBlockConns,
            re.OnClientEvent:Connect(function(connectionId, packedArgs)
                rbOnPacket(connectionId, packedArgs)
            end)
        )
    end

    -- 2. UnreliableRemoteEvent — fast/frequent events, attacks may come here
    local ure = netPath:FindFirstChild("UnreliableRemoteEvent")
    if ure then
        table.insert(remoteBlockConns,
            ure.OnClientEvent:Connect(function(connectionId, packedArgs)
                rbOnPacket(connectionId, packedArgs)
            end)
        )
    end

    -- 3. BindableEvent (named "Event") — server→localscript signals
    local be = netPath:FindFirstChild("Event")
    if be then
        table.insert(remoteBlockConns,
            be.Event:Connect(function(connectionId, packedArgs)
                rbOnPacket(connectionId, packedArgs)
            end)
        )
    end

    -- Pre-warm the registry decode cache
    task.spawn(rbGetRegistry)
end
...
-- FIXED: Sound Block toggle now properly sets the flag and hooks sounds
secCore:Toggle({ 
    Title="Sound Block", 
    Type="Checkbox", 
    ...
    Callback=function(v) 
        ...
        if v then
            ...
            -- Force re-hook all existing sounds
            pcall(function()
                local kf = getTeamFolder("Killers")
                if kf then
                    for _, killer in pairs(kf:GetChildren()) do
                        for _, d in pairs(killer:GetDescendants()) do
                            if d:IsA("Sound") then
                                -- Clear any existing hooks for this sound
...
-- Test button to debug sound detection
secCore:Button({ 
...
secCore:Toggle({ Title="Remote Auto Block", Type="Checkbox", Flag="guestRemoteBlockEnabled", Default=false,
    Callback=function(v)
        guestCfg.remoteBlockEnabled = v
        if v then
            setupRemoteBlock()
        else
            for _, c in ipairs(remoteBlockConns) do pcall(function() c:Disconnect() end) end
            remoteBlockConns = {}
        end
    end })
...
secCore:Button({ 
    ...
    Callback = function()
        if guestCfg.remoteBlockEnabled then
            setupRemoteBlock()
        end
