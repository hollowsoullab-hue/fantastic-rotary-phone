-- BetrayalHub - Complete Final Version (ALL FEATURES FILLED IN)
...
-- ============================================================
-- IMPROVED AUTO BLOCK SYSTEM (From Meowsaken)
-- ============================================================

-- Animation IDs from Meowsaken
...
local KillersFolder = svc.WS:WaitForChild("Players"):WaitForChild("Killers")

-- FACING CHECK
local function ab_isFacingMe(targetRoot, myRoot)
    if not autoBlock.facingCheck then return true end
    if not targetRoot or not myRoot then return false end

    local targetLook = targetRoot.CFrame.LookVector
    local dirToMe = (myRoot.Position - targetRoot.Position).Unit
    local dot = targetLook:Dot(dirToMe)

    return dot >= autoBlock.facingDot
end

-- WALL CHECK
local function ab_checkWall(targetRoot)
    local char = lp.Character
    local hrp = char and char:FindFirstChild("HumanoidRootPart")
    if not hrp or not targetRoot then return false end
    
    local raycastParams = RaycastParams.new()
    raycastParams.FilterType = Enum.RaycastFilterType.Exclude
    raycastParams.FilterDescendantsInstances = {char, targetRoot.Parent}
    raycastParams.IgnoreWater = true
    
    local direction = targetRoot.Position - hrp.Position
    local raycastResult = svc.WS:Raycast(hrp.Position, direction, raycastParams)
    return raycastResult == nil
end

-- REMOTE FUNCTIONS
local function ab_remoteBlock()
    local Event = svc.RS:FindFirstChild("Modules")
    if Event then Event = Event:FindFirstChild("Network") end
    if Event then Event = Event:FindFirstChild("Network") end
    if Event then Event = Event:FindFirstChild("RemoteEvent") end
...
-- AIM PUNCH
local function ab_aimAtTarget(targetRoot)
    if not targetRoot then return end
    autoBlock.aimTarget = targetRoot
    
    if not autoBlock.isAiming then
        autoBlock.isAiming = true
        task.spawn(function()
            local startTime = os.clock()
            while os.clock() - startTime < 0.7 do
                local char = lp.Character
                local hrp = char and char:FindFirstChild("HumanoidRootPart")
                if hrp and autoBlock.aimTarget and autoBlock.aimTarget.Parent then
                    hrp.CFrame = CFrame.new(hrp.Position, Vector3.new(autoBlock.aimTarget.Position.X, hrp.Position.Y, autoBlock.aimTarget.Position.Z))
                end
                svc.Run.RenderStepped:Wait()
            end
            autoBlock.isAiming = false
            autoBlock.aimTarget = nil
        end)
    end
end

-- GET CLOSEST KILLER
local function ab_getClosestKillerRoot()
    ...
    if myRoot then
        for _, killer in ipairs(KillersFolder:GetChildren()) do
            ...
            if root then
                ...
                if distance < shortestDistance and distance <= autoBlock.detectionRange then
                    if autoBlock.wallCheck and not ab_checkWall(root) then continue end
...
-- SETUP PREDICTIVE BLOCK
local function ab_setupPredictiveBlock(humanoid, targetRoot)
    if humanoid:FindFirstChild("BlockPredictSetup") then return end
    ...
    humanoid.AnimationPlayed:Connect(function(track)
        ...
        if anim and anim.AnimationId then
            ...
            if dist <= autoBlock.detectionRange then
                if autoBlock.wallCheck and not ab_checkWall(targetRoot) then return end
                if not ab_isFacingMe(targetRoot, myRoot) then return end

                if trustedAnimationIds[id] or 
                   animName:find("slash") or 
                   animName:find("attack") or 
                   animName:find("swing") or 
                   (track.Length >= 0.3 and track.Length <= 3 and animName:find("hit")) then
                    
                    if autoBlock.blockDelay > 0 then 
                        task.wait(autoBlock.blockDelay) 
                    end
                    ab_remoteBlock()
                end
            end
...
-- SETUP AIM PUNCH
local function ab_setupAimPunch(humanoid)
    humanoid.AnimationPlayed:Connect(function(track)
        if not autoBlock.aimPunch then return end
        
        local anim = track.Animation
        if anim and anim.AnimationId then
            local id = string.match(anim.AnimationId, "%d+")
            
            if aimPunchAnimationIds[id] then
                local targetRoot = ab_getClosestKillerRoot()
                if targetRoot then
                    ab_aimAtTarget(targetRoot)
                end
            end
        end
    end)
...
-- HITBOX DRAGGING
local function ab_getNearestKillerModel()
    local myChar = lp.Character
    local myRoot = myChar and myChar:FindFirstChild("HumanoidRootPart")
...
local function ab_getKillerHRP(killerModel)
...
local function ab_beginDragIntoKiller(killerModel)
    if autoBlock.hdtDebounce then return end
    ...
    local targetHRP = ab_getKillerHRP(killerModel)
    if not targetHRP then return end

    autoBlock.hdtDebounce = true
    ...
    bv.MaxForce = Vector3.new(3e6, 0, 3e6)
    bv.Velocity = Vector3.zero
    bv.Parent = hrp

    local conn
    conn = svc.Run.Heartbeat:Connect(function()
        if not autoBlock.hdtDebounce or not char.Parent or not killerModel.Parent then
            conn:Disconnect()
            bv:Destroy()
        ...
        targetHRP = ab_getKillerHRP(killerModel)
        if not targetHRP then
            autoBlock.hdtDebounce = false
            return
        end
        ...
        if horiz.Magnitude > 2.0 then
            bv.Velocity = horiz.Unit * autoBlock.hdtSpeed
        else
            bv.Velocity = Vector3.zero
            autoBlock.hdtDebounce = false
        end
...
-- REFRESH ANIMATOR
local function ab_refreshAnimator()
    local char = lp.Character
    local hum = char and char:FindFirstChildOfClass("Humanoid")
    autoBlock.cachedAnimator = hum and hum:FindFirstChildOfClass("Animator") or nil
end
...
svc.Run.RenderStepped:Connect(function()
    if not autoBlock.hdtEnabled then return end
    if not autoBlock.cachedAnimator then ab_refreshAnimator() end
    local animator = autoBlock.cachedAnimator
    ...
    for _, track in ipairs(animator:GetPlayingAnimationTracks()) do
        ...
        if animId and table.find(blockAnimIds, animId) then
            if (track.TimePosition or 0) <= 0.12 then
                local nearest = ab_getNearestKillerModel()
                if nearest then
                    task.wait(autoBlock.hdtDelay)
                    task.spawn(ab_beginDragIntoKiller, nearest)
                end
            end
        end
    end
...
-- ANTI-FLICK ZONE DETECTION
task.spawn(function()
    local overlapParams = OverlapParams.new()
    overlapParams.FilterType = Enum.RaycastFilterType.Include
    local HITBOX_DETECT_RADIUS = 15
    
    while true do
        task.wait(0.1)
        if not autoBlock.hdtEnabled then continue end

        local char = lp.Character
        local myRoot = char and char:FindFirstChild("HumanoidRootPart")
        if not myRoot or autoBlock.hdtDebounce then continue end

        local parts = svc.WS:GetPartBoundsInRadius(myRoot.Position, HITBOX_DETECT_RADIUS, overlapParams)
        local found = false
        for _, part in ipairs(parts) do
            if part.Name == "AntiFlickZone" then
                found = true
                break
            end
        end
...
svc.Run.RenderStepped:Connect(function()
    ...
    for _, killer in ipairs(KillersFolder:GetChildren()) do
        ...
        if root and humanoid then
            if not humanoid:FindFirstChild("BlockPredictSetup") then
                ab_setupPredictiveBlock(humanoid, root)
            end
            if not humanoid:FindFirstChild("AimPunchSetup") and autoBlock.aimPunch then
                local tag = Instance.new("BoolValue")
                tag.Name = "AimPunchSetup"
                tag.Parent = humanoid
                ab_setupAimPunch(humanoid)
            end
        end
    end
end)
...
-- PARRY HOOK
local network = require(svc.RS:WaitForChild("Modules"):WaitForChild("Network"):WaitForChild("Network"))
network:SetConnection(("%*1337ParryIcon"):format(lp.Name), "REMOTE_EVENT", function(blocked)
    if blocked and autoBlock.autoPunch then
        ab_remotePunch()
    end
end)

-- ============================================================
-- VEERONICA FEATURES (PC Space Bar)
-- ============================================================
...
local function veeronica_getBehaviorFolder()
...
local function veeronica_getSprintingButton()
    return lp.PlayerGui:WaitForChild("MainUI"):WaitForChild("SprintingButton")
end

local veeronica_behaviorFolder = veeronica_getBehaviorFolder()

local function veeronica_safeConnectPropertyChanged(instance, prop, fn)
...
local function veeronica_doTrick()
    pcall(function()
        local ok, btn = pcall(veeronica_getSprintingButton)
        if ok and btn then
        ...
        if remote then
            remote:FireServer("UseActorAbility", {
                (function(bytes)
                    local b = buffer.create(#bytes)
                    for i = 1, #bytes do
                        buffer.writeu8(b, i - 1, bytes[i])
                    ...
                    return b
                end)({ 3, 5, 0, 0, 0, 84, 114, 105, 99, 107 })
            })
        end
    end)
end

local function veeronica_setupSpaceBar()
    if veeronica.spaceBarConn then
        veeronica.spaceBarConn:Disconnect()
        veeronica.spaceBarConn = nil
    end
    
    veeronica.spaceBarConn = svc.Input.InputBegan:Connect(function(input, gameProcessed)
        if gameProcessed then return end
        if not veeronica.autoTrick then return end
        
        if input.KeyCode == Enum.KeyCode.Space then
            veeronica.isHoldingSpace = true
            veeronica_doTrick()
            
            task.spawn(function()
                local startTime = tick()
                while veeronica.isHoldingSpace and (tick() - startTime) < 1.5 do
                    if veeronica.autoTrick then
                        veeronica_doTrick()
                    end
                    task.wait(0.1)
                end
    ...
    svc.Input.InputEnded:Connect(function(input)
        if input.KeyCode == Enum.KeyCode.Space then
            veeronica.isHoldingSpace = false
        end
    end)
end

local function veeronica_monitorHighlight(h)
    if not h or veeronica.activeMonitors[h] then return end
    ...
    local function cleanup()
        ...
        veeronica.activeMonitors[h] = nil
    end
    ...
    local function onChanged()
        ...
        if prevState ~= currState and currState then
            local device = svc.Input:GetDeviceType()
            if device == Enum.UserInputType.Touch then
                local ok, btn = pcall(veeronica_getSprintingButton)
                if ok and btn then
                    for _, v in pairs(getconnections(btn.MouseButton1Down)) do
                        pcall(v.Fire, v)
    ...
    local c = veeronica_safeConnectPropertyChanged(h, "Adornee", onChanged)
    ...
    veeronica.activeMonitors[h] = cleanup
    task.spawn(onChanged)
end

local function veeronica_startManager()
    if veeronica.descendantAddedConn then return end
    
    for _, desc in ipairs(veeronica_behaviorFolder:GetDescendants()) do
        if desc:IsA("Highlight") then veeronica_monitorHighlight(desc) end
    end
    
    veeronica.descendantAddedConn = veeronica_behaviorFolder.DescendantAdded:Connect(function(child)
        if child:IsA("Highlight") then veeronica_monitorHighlight(child) end
    end)
end

local function veeronica_stopManager()
    if veeronica.descendantAddedConn and veeronica.descendantAddedConn.Connected then
        veeronica.descendantAddedConn:Disconnect()
    end
    veeronica.descendantAddedConn = nil
    
    for _, cleanup in pairs(veeronica.activeMonitors) do pcall(cleanup) end
    veeronica.activeMonitors = {}
end

local function veeronica_setAutoTrick(enabled)
    if veeronica.autoTrick == enabled then return end
    veeronica.autoTrick = enabled
    
    if enabled then
        veeronica_startManager()
        veeronica_setupSpaceBar()
        print("[Veeronica] 🚀 Auto Trick ENABLED - Press SPACE BAR for tricks!")
    else
        veeronica_stopManager()
        if veeronica.spaceBarConn then
            veeronica.spaceBarConn:Disconnect()
            veeronica.spaceBarConn = nil
        end
        print("[Veeronica] ❌ Auto Trick DISABLED")
    end
end
...
-- ============================================================
-- GUEST 1337 - HDT TAB (OPTIMIZED - NO LAG)
-- ============================================================
...
function applyConfigFromFlags()
    pcall(function()
        ...
        if win.Flags.jdCrystalTracking ~= nil then jd_crystalTracking=win.Flags.jdCrystalTracking; if jd_crystalTracking and jd_crystalAutoHold then jd_startIndicatorTracking() elseif not jd_crystalTracking then jd_stopIndicatorTracking() end end
