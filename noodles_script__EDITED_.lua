...
local function jd_rotateIndicatorToKiller()
    ...
    local targetPos = Vector3.new(predPos.X, predPos.Y + jd_AIM_OFFSET, predPos.Z)
    ...
    if dist > 0.1 then
        ...
        -- Just move the EndPoint part directly — beam follows it automatically
        jd_cachedEndPoint.CFrame = CFrame.new(newPos)
    end
                    ...
                    if val == 1 then
                        ...
                        task.spawn(function()
                            pcall(function()
                                ...
                                jd_forceMaxCharge()
                                -- Update the charge bar UI to show 100% instantly
                                local playerGui = svc.Players.LocalPlayer.PlayerGui
                                if playerGui then
                                    local tempUI = playerGui:FindFirstChild("TemporaryUI")
                                    if tempUI then
                                        ...
                                        if chargeUI then
                                            local bar = chargeUI:FindFirstChild("Bar")
                                            if bar then
                                                local fill = bar:FindFirstChild("Fill") or bar:FindFirstChild("Percent") or bar:FindFirstChildOfClass("Frame") or bar:FindFirstChildOfClass("ImageLabel")
                                                if fill then
                                                    if fill:IsA("Frame") or fill:IsA("ImageLabel") then
