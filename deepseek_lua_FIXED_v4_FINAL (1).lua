...
local function g_attemptBlockForSound(sound, preId)
    ...
    print("[SOUND BLOCK] Detected sound " .. id .. " from " .. killer.Name)
    
    if guestCfg.blockDelay and guestCfg.blockDelay > 0 then
        print("[SOUND BLOCK] Delaying block: " .. guestCfg.blockDelay .. "s")
        task.wait(guestCfg.blockDelay)
    end
    
    print("[SOUND BLOCK] Blocking now!")
    g_fireBlock()
    g.lastBlockTime = tick()
    if guestCfg.autoPunchOn then 
        task.delay(guestCfg.autoPunchDelay, g_firePunch)
        print("[SOUND BLOCK] Auto-punching after: " .. guestCfg.autoPunchDelay .. "s")
    end
...
local function g_hookSound(sound)
    if not sound or not sound:IsA("Sound") or g.soundHooks[sound] then 
        if sound and sound:IsA("Sound") then
            print("[SOUND HOOK] Already hooked: " .. sound.Name)
        end
        return 
    end
    local preId = g_extractId(sound)
    if not preId then 
        print("[SOUND HOOK] No valid ID for: " .. sound.Name)
        return 
    end
    
    print("[SOUND HOOK] Hooking sound: " .. sound.Name .. " (ID: " .. preId .. ")")
    
    local pc = sound.Played:Connect(function()
        print("[SOUND PLAYED] " .. preId .. " - IsPlaying: " .. tostring(sound.IsPlaying))
        if guestCfg.autoBlockOn then
            print("[SOUND DETECT] AutoBlock enabled, spawning block check...")
            print("[SOUND TRIGGER] Spawning block check for " .. preId)
            task.spawn(g_attemptBlockForSound, sound, preId)
        else
            print("[SOUND DETECT] AutoBlock disabled")
        end
    end)
    
    local pp = sound:GetPropertyChangedSignal("IsPlaying"):Connect(function()
        print("[SOUND PROP CHANGED] " .. preId .. " - IsPlaying: " .. tostring(sound.IsPlaying))
        if sound.IsPlaying and guestCfg.autoBlockOn then
            print("[SOUND DETECT] Sound now playing, spawning block check...")
            print("[SOUND TRIGGER] Spawning block check for " .. preId)
            task.spawn(g_attemptBlockForSound, sound, preId)
        end
    end)
    ...
    print("[SOUND HOOKED] Connected 3 events for " .. preId)
    
    if sound.IsPlaying then
        print("[SOUND CHECK] Sound already playing on hook - spawning block check")
        task.spawn(g_attemptBlockForSound, sound, preId)
    end
end
...
local function g_hookExistingSounds()
    ...
    if kf then
        ...
        print("[SOUND HOOK] Found and hooked " .. count .. " existing sounds")
    else
        print("[SOUND HOOK] No Killers folder found")
    end
end
...
local function g_setupSoundHooks()
    task.spawn(function()
        local pf = svc.WS:FindFirstChild("Players")
        if not pf then 
            print("[SOUND SETUP] Waiting for Players folder...")
            pf = svc.WS:WaitForChild("Players", 30) 
        end
        if not pf then 
            print("[SOUND SETUP] Players folder timeout!")
            return 
        end
        
        local kf = pf:FindFirstChild("Killers")
        if not kf then 
            print("[SOUND SETUP] Waiting for Killers folder...")
            kf = pf:WaitForChild("Killers", 30) 
        end
        if not kf then 
            print("[SOUND SETUP] Killers folder timeout!")
            return 
        end
        
        print("[SOUND SETUP] Found Killers folder, hooking existing sounds...")
        g_hookExistingSounds()
        
        print("[SOUND SETUP] Connecting to DescendantAdded...")
        kf.DescendantAdded:Connect(function(d)
            if d:IsA("Sound") then 
                print("[SOUND NEW] New sound added: " .. d.Name)
                pcall(g_hookSound, d) 
            end
        end)
        
        print("[SOUND SETUP] Connecting to ChildAdded...")
        kf.ChildAdded:Connect(function(k)
            print("[SOUND NEW] New killer added: " .. k.Name)
            task.wait(0.1)
            for _, d in pairs(k:GetDescendants()) do
                if d:IsA("Sound") then 
                    print("[SOUND NEW] Found sound in new killer: " .. d.Name)
                    pcall(g_hookSound, d) 
                end
            end
        end)
        
        print("[SOUND SETUP] Sound hook system initialized!")
    end)
end
...
local function g_createVisionPart()
    ...
    p.Size = Vector3.new(1,1,1); p.Shape = Enum.PartType.Block
    p.Anchored = true; p.CanCollide = false
    p.Transparency = 0.7; p.Color = Color3.fromRGB(255,0,0)
...
local function g_updateVision()
    ...
    -- Calculate trapezoid front/back
    local killerPos = origin.Position
    local frontPos = killerPos + fwd * range
    local lFront = frontPos + lDir * range * 0.5
    local rFront = frontPos + rDir * range * 0.5
    
    -- Center and dimensions of the trapezoid
    local center = (killerPos + frontPos) / 2
    local height = (frontPos - killerPos).Magnitude
    local width = (rFront - lFront).Magnitude
    
    -- Create main trapezoid part
    if not g.centerPart or not g.centerPart.Parent then 
        g.centerPart = g_createVisionPart() 
    end
    
    g.centerPart.Size = Vector3.new(width, range * 0.75, height)
    g.centerPart.Position = center
    g.centerPart.CFrame = CFrame.new(center, frontPos)
    
    -- Destroy old cone parts
    if g.leftPart and g.leftPart.Parent then g.leftPart:Destroy() end
    if g.rightPart and g.rightPart.Parent then g.rightPart:Destroy() end
    if g.leftToKillerPart and g.leftToKillerPart.Parent then g.leftToKillerPart:Destroy() end
    if g.rightToKillerPart and g.rightToKillerPart.Parent then g.rightToKillerPart:Destroy() end
    if g.centerToKillerPart and g.centerToKillerPart.Parent then g.centerToKillerPart:Destroy() end 
end
...
local TRIGGER_ANIMS = {
    ...
    ["123172382755876"]=true, -- NEW ANIMATION ID
}
        ...
        for _, killer in ipairs(kf:GetChildren()) do
            ...
            for _, track in ipairs(tracks) do
                ...
                if animId and TRIGGER_ANIMS[animId] then
                    print("[ANIM BLOCK] Detected animation: " .. animId .. " from " .. killer.Name)
                    local now = tick()
                    if now - blockLastTime >= BLOCK_CD then
                        blockLastTime = now
                        triggered = true
                        local function doBlock()
                            print("[ANIM BLOCK] Blocking with: " .. guestCfg.blockType)
                            fireAbility(guestCfg.blockType)
                            if guestCfg.doubleBlock then
                                task.wait(0.05)
                                fireAbility("Punch")
                                print("[ANIM BLOCK] Double block with Punch")
                            end
                        end
                        if guestCfg.blockDelay > 0 then
                            print("[ANIM BLOCK] Delayed block: " .. guestCfg.blockDelay .. "s")
...
-- Body parts that must NEVER be removed
local lm4_bodyParts = {
...
-- Cosmetic accessory names found on TwoTime's character
local lm4_cosmeticNames = {
    ["Backhair"]=true, ["Bangs"]=true, ["Bow"]=true, ["Dress"]=true,
    ["Hat"]=true, ["NeckLace"]=true, ["Tail"]=true, ["Wings"]=true,
    ["LegacyMS4_Attached"]=true,
}
...
local function lm4_instantlyRemoveAccessories(character)
    ...
    for _, child in ipairs(character:GetChildren()) do
        -- Never touch body parts or humanoid
        if lm4_bodyParts[child.Name] then continue end
        if child:IsA("Humanoid") or child:IsA("Motor6D") or child:IsA("Script") or child:IsA("LocalScript") then continue end
        -- Never touch the dagger/weapon
        if lm4_isDaggerOrWeapon(child) then continue end
        -- Remove known cosmetics, Accessory/Hat/Clothing instances, and our own attached folder
        if lm4_cosmeticNames[child.Name]
            or child:IsA("Accessory") or child:IsA("Hat")
            or child:IsA("Shirt") or child:IsA("Pants")
            or child:IsA("ShirtGraphic") or child:IsA("Clothing") then
...
local function lm4_start()
    ...
    -- Reset Life state when a new round starts (Map is deleted = round ends, new Map appears = new round)
    local function lm4_watchForNewRound()
        local function watchWorkspace()
            table.insert(lm4_connections, svc.WS.ChildRemoved:Connect(function(child)
                if child.Name == "Map" then
                    lm4_hasUsedLife2 = false
                    print("[TwoTime] Map deleted - new round detected, reset to Life1")
                end
            end))
            table.insert(lm4_connections, svc.WS.ChildAdded:Connect(function(child)
                if child.Name == "Map" then
                    task.wait(0.2)
                    lm4_hasUsedLife2 = false
                    print("[TwoTime] New Map detected - reset to Life1")
                end
            end))
        end
        watchWorkspace()
    ...
    table.insert(lm4_connections, lp.CharacterAdded:Connect(function(char)
        -- Remove accessories immediately and multiple times to catch delayed spawns
        lm4_instantlyRemoveAccessories(char)
        task.wait(0.1); lm4_instantlyRemoveAccessories(char)
        task.wait(0.2); lm4_instantlyRemoveAccessories(char)
        task.wait(0.3); lm4_instantlyRemoveAccessories(char)
        
        -- Monitor and continuously remove new accessories that spawn in
        local cleanupConnection
        cleanupConnection = char.ChildAdded:Connect(function(child)
            if not char or not char.Parent then cleanupConnection:Disconnect(); return end
            -- Check if it's an accessory, clothing, or known cosmetic
            if child:IsA("Accessory") or child:IsA("Hat") or child:IsA("Shirt") 
                or child:IsA("Pants") or child:IsA("ShirtGraphic") or child:IsA("Clothing")
                or lm4_cosmeticNames[child.Name] then
                pcall(function() child:Destroy() end)
                print("[TwoTime] Removed newly spawned: " .. child.Name)
            end
        end)
        table.insert(lm4_connections, cleanupConnection)
    end))
    ...
    table.insert(lm4_connections, lp.CharacterAdded:Connect(function(char)
        ...
        -- Aggressive cleanup before applying skin
        lm4_instantlyRemoveAccessories(character)
        task.wait(0.1); lm4_instantlyRemoveAccessories(character)
        
        if respawnPart and lm4_isAtRespawn(character, respawnPart) then
            -- AT RESPAWN LOCATION - aggressive cleanup
            print("[TwoTime] Detected at respawn location - aggressive cleanup")
            for _ = 1, 5 do
                lm4_instantlyRemoveAccessories(character)
                task.wait(0.05)
            end
            ...
            -- NOT at respawn - normal flow
            if not lm4_hasUsedLife2 then
                lm4_hasUsedLife2 = true; lm4_applySkin()
            end
        end
        
        -- Final cleanup after skin is applied
        task.wait(0.1); lm4_instantlyRemoveAccessories(character)
    end))
    ...
    -- Monitor respawn location and clean up accessories there
    local respawnMonitorThread = task.spawn(function()
        while lm4_active do
            pcall(function()
                if lp.Character and respawnPart then
                    if lm4_isAtRespawn(lp.Character, respawnPart) then
                        -- At respawn - ensure accessories are gone
                        ...
                        for _, child in ipairs(lp.Character:GetChildren()) do
                            if (child:IsA("Accessory") or child:IsA("Hat") or 
                                ...
                                child:IsA("ShirtGraphic") or child:IsA("Clothing")) then
                                hasAccessories = true
                                break
                            end
                        end
                        if hasAccessories then
                            print("[TwoTime] Found accessories at respawn - cleaning...")
                            lm4_instantlyRemoveAccessories(lp.Character)
                        end
                    end
                end
            end)
    ...
    table.insert(lm4_connections, respawnMonitorThread)
