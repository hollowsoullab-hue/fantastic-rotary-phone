...
-- ============================================================
-- DIRECT NETWORK AUTO-BLOCK (NEW & IMPROVED)
-- ============================================================
...
local networkBlock = {
    ...
    originalSetConnection = nil,
    originalOnClientEvent = nil,
    blockCooldown = 0.35,
    lastBlockTime = 0,
    hookActive = false,
    remoteBlockCache = {},
    abilityPatterns = {"Slash", "Punch", "Stab"},
    triggeredAbilities = {
        ["Slash"] = true,
        ["Punch"] = true,
        ["Stab"] = true,
...
local function networkBlock_isBlockReady()
    ...
    return tick() - networkBlock.lastBlockTime >= networkBlock.blockCooldown
end

local function networkBlock_fireBlock()
    local network = networkBlock.networkModule
    if not network then 
        network = getNetwork()
        if not network then return end
        networkBlock.networkModule = network
    end
    
    pcall(function()
        -- Try multiple methods to fire block
        local blockBuffer = buffer.fromstring("\x03\x05\x00\x00\x00Block")
        
        -- Method 1: Via Network module
        if network.FireServerConnection then
            network:FireServerConnection("UseActorAbility", "REMOTE_EVENT", {[1] = blockBuffer})
            return
        end
        
        -- Method 2: Via RemoteEvent directly
        local remote = hbGetRemote()
        if remote then
            remote:FireServer("UseActorAbility", {[1] = blockBuffer})
            return
        end
        
        -- Method 3: Via raw RemoteEvent
        local rs = svc.RS
        local modules = rs:FindFirstChild("Modules")
        if modules then
            local networkFolder = modules:FindFirstChild("Network")
            if networkFolder then
                local networkChild = networkFolder:FindFirstChild("Network")
                if networkChild then
                    local remoteEvent = networkChild:FindFirstChild("RemoteEvent")
                    if remoteEvent then
                        remoteEvent:FireServer("UseActorAbility", {[1] = blockBuffer})
                    end
                end
            end
        end
...
local function networkBlock_extractAbilityName(data)
    local name = ""
    pcall(function()
...
local function networkBlock_isTriggeredAbility(name)
    if not name or name == "" then return false end
    
    -- Check exact matches
    if networkBlock.triggeredAbilities[name] then
        return true
    end
    
    -- Check patterns
    for _, pattern in ipairs(networkBlock.abilityPatterns) do
        if name:find(pattern) or name:lower():find(pattern:lower()) then
            return true
        end
    end
...
-- Get the internal connection registry
local function networkBlock_getConnectionRegistry()
    local network = networkBlock.networkModule
    if not network then return nil end
    
    -- Try to get the upvalue v_u_21 from SetConnection
    local ok, registry = pcall(function()
        local info = debug.getinfo(network.SetConnection)
        local upvalues = {}
        for i = 1, info.nups or 0 do
            local name, value = debug.getupvalue(network.SetConnection, i)
...
local function networkBlock_hookNetwork()
    local network = networkBlock.networkModule
    if not network then 
        network = getNetwork()
        if not network then 
            warn("[Network Block] Could not find Network module")
            return false 
        end
        networkBlock.networkModule = network
    end
    
    if networkBlock.hookActive then
        return true
    end
    
    print("[Network Block] 🪝 Hooking into Network module...")
    
    -- ===== HOOK 1: FireServerConnection =====
    -- This catches ALL outgoing server calls
    if network.FireServerConnection then
        local originalFireServer = network.FireServerConnection
        
        network.FireServerConnection = function(self, connectionName, remoteType, ...)
            -- Intercept UseActorAbility calls
            if connectionName == "UseActorAbility" then
                local args = {...}
                if args[1] and args[1][1] then
                    local abilityName = networkBlock_extractAbilityName(args[1][1])
                    
                    if networkBlock_enabled and networkBlock_isTriggeredAbility(abilityName) then
                        -- Check if this is from a killer (only block enemy abilities)
                        local isKillerAbility = false
                        pcall(function()
                            if args[2] and args[2]:IsA("Model") then
                                local kf = getTeamFolder("Killers")
                                if kf and args[2]:IsDescendantOf(kf) then
                                    isKillerAbility = true
                                end
                            end
                        end)
                        
                        -- If from killer and block is ready, block instead
                        if isKillerAbility and networkBlock_isBlockReady() then
                            networkBlock.lastBlockTime = tick()
                            if networkBlock.debugMode then
                                print("[Network Block] 🛡️ BLOCKED: " .. abilityName .. " (outgoing)")
                            end
                            
                            -- Fire block instead of the original ability
                            local blockArgs = {[1] = buffer.fromstring("\x03\x05\x00\x00\x00Block")}
                            return originalFireServer(self, connectionName, remoteType, blockArgs)
                        end
                    end
                end
            end
            
            -- Let other calls through normally
            return originalFireServer(self, connectionName, remoteType, ...)
        end
        
        networkBlock.originalFireServer = originalFireServer
        print("[Network Block] ✅ Hooked FireServerConnection")
    end
    
    -- ===== HOOK 2: SetConnection (Connection Registry) =====
    -- This intercepts how the game handles incoming remote events
    if network.SetConnection then
        local originalSetConnection = network.SetConnection
        
        network.SetConnection = function(self, name, type, callback)
            -- Wrap callbacks for UseActorAbility
            if name == "UseActorAbility" or name:find("UseActorAbility") then
                local wrappedCallback = function(player, ...)
                    ...
                    if args[1] and args[1][1] then
                        ...
                        if networkBlock_enabled and networkBlock_isTriggeredAbility(abilityName) then
                            ...
                            -- If from killer, block it
                            if isKiller and networkBlock_isBlockReady() then
                                networkBlock.lastBlockTime = tick()
                                if networkBlock.debugMode then
                                    print("[Network Block] 🛡️ BLOCKED: " .. abilityName .. " (incoming)")
                                end
                                
                                -- Fire block
                                task.spawn(networkBlock_fireBlock)
                                
                                -- Don't process the original ability
                                return
                            end
                        end
                    end
                    
                    -- Process normally if not blocked
                    if callback then
                        return callback(player, ...)
                    end
                end
                
                return originalSetConnection(self, name, type, wrappedCallback)
            end
            
            return originalSetConnection(self, name, type, callback)
        end
        
        networkBlock.originalSetConnection = originalSetConnection
        print("[Network Block] ✅ Hooked SetConnection")
    end
    
    -- ===== HOOK 3: Direct RemoteEvent OnClientEvent =====
    -- This is the most reliable method - hooks the actual event
    local remote = hbGetRemote()
    if remote then
        -- Store original connections
        local eventConn = remote.OnClientEvent:Connect(function(...)
            local args = {...}
            if args[1] == "UseActorAbility" and args[2] and args[2][1] then
                local abilityName = networkBlock_extractAbilityName(args[2][1])
                
                if networkBlock_enabled and networkBlock_isTriggeredAbility(abilityName) then
                    -- Check if from killer
                    local isKiller = false
                    pcall(function()
                        if args[2] and args[2][2] and args[2][2]:IsA("Model") then
                            local kf = getTeamFolder("Killers")
                            if kf and args[2][2]:IsDescendantOf(kf) then
                                isKiller = true
                            end
                        end
                    end)
                    ...
                    if isKiller and networkBlock_isBlockReady() then
                        networkBlock.lastBlockTime = tick()
                        if networkBlock.debugMode then
                            print("[Network Block] 🛡️ BLOCKED: " .. abilityName .. " (event)")
                        end
                        task.spawn(networkBlock_fireBlock)
                    end
                end
        ...
        table.insert(networkBlock.remoteConnections, eventConn)
        print("[Network Block] ✅ Hooked RemoteEvent.OnClientEvent")
    end
    
    networkBlock.hookActive = true
    print("[Network Block] ✅ All hooks active!")
    return true
end

local function networkBlock_unhookNetwork()
    local network = networkBlock.networkModule
    if not network then return end
    
    -- Restore FireServerConnection
    if network.FireServerConnection and networkBlock.originalFireServer then
        network.FireServerConnection = networkBlock.originalFireServer
        networkBlock.originalFireServer = nil
    end
    
    -- Restore SetConnection
    if network.SetConnection and networkBlock.originalSetConnection then
        network.SetConnection = networkBlock.originalSetConnection
        networkBlock.originalSetConnection = nil
    end
    
    -- Disconnect remote connections
    for _, conn in ipairs(networkBlock.remoteConnections) do
        pcall(function() conn:Disconnect() end)
    end
    networkBlock.remoteConnections = {}
    
    networkBlock.hookActive = false
    print("[Network Block] ❌ Hooks removed")
end

local function networkBlock_start()
    if networkBlock.hookActive then
        return
    end
    
    networkBlock.networkModule = getNetwork()
    if not networkBlock.networkModule then
        warn("[Network Block] Waiting for Network module...")
        task.delay(0.5, networkBlock_start)
        return
    end
    
    if networkBlock.enabled then
        networkBlock_hookNetwork()
    end
end

local function networkBlock_stop()
    networkBlock.enabled = false
    networkBlock_unhookNetwork()
end

local function networkBlock_toggle(on)
    networkBlock.enabled = on
    if on then
        networkBlock_start()
    else
        networkBlock_stop()
...
-- Re-hook when character changes
...
local function musicFetchAsync(name, cb)
    ...
    task.spawn(function()
        ...
        musicFetchInFlight[name] = nil        if asset and cb then cb(asset) end
    end)
end
...
do
    local sec_chance = tabChance:Section({ Title = "Chance Aimbot", Opened = true })
    ...
    local function chanceHookAnimator(char)
        ...
        local chanceTriggers={
            ["133607163653602"]=true,["133491532453922"]=true,["131189930305001"]=true,
            ["111384272984267"]=true,["103601716322988"]=true,["76649505662612"]=true
        }
            ...
            if id and chanceTriggers[id] then
                ...
                track.Ended:Connect(function()
                    ...
                    if chanceBodyGyro and chanceBodyGyro.Parent then
                        chanceBodyGyro:Destroy(); chanceBodyGyro = nil
                    end
                    chanceAiming = false
                end)
            end
    ...
    svc.Input.InputBegan:Connect(function(input, gpe)
        ...
        if chanceHoldToAim and input.KeyCode==chanceAimKey then
            chanceHoldingKey=true; chanceAiming=true; chanceStartTime=tick()
        end
    end)
    svc.Input.InputEnded:Connect(function(input)
        if chanceHoldToAim and input.KeyCode==chanceAimKey then
            chanceHoldingKey=false; chanceAiming=false
        end
    end)
    ...
    svc.Run.RenderStepped:Connect(function(dt)
        ...
        if chanceHoldToAim then
            ...
            if tick()-chanceStartTime>chanceAimDuration then chanceAiming=false; return end
        end
    ...
    -- UI
    sec_chance:Toggle({ Title = "Enable Aimbot", Flag = "chanceAimOn", Default = false,
        Callback = function(v)
            chanceAimEnabled = v
            if not v then
    ...
    sec_chance:Dropdown({ Title = "Prediction Mode", Flag = "chancePredMode",
        Values = {"Velocity","Ping","Look","LookPing"}, Value = "Velocity",
        Callback = function(v) chancePredMode=v end })

    sec_chance:Input({ Title = "Prediction Value", Flag = "chancePredVal", Placeholder = "0.5",
        Callback = function(v) local n=tonumber(v); if n then chancePredValue=n end end })

    sec_chance:Slider({ Title = "Smooth Speed", Flag = "chanceSmoothSpd",
        Value = {Min=1,Max=30,Default=14}, Step = 1,
        Callback = function(v) chanceSmoothSpeed=v end })

    sec_chance:Toggle({ Title = "Height-Aware Aim", Flag = "chanceHeightAim", Default = true,
        Callback = function(v) chanceHeightAim=v end, Type = "Checkbox"})

    sec_chance:Dropdown({ Title = "Aim Behavior", Flag = "chanceAimBehav",
        Values = {"Normal","360"}, Value = "Normal",
        Callback = function(v) chanceAimBehavior=v end })

    sec_chance:Input({ Title = "Spin Duration", Flag = "chanceSpinDur", Placeholder = "0.5",
        Callback = function(v) local n=tonumber(v); if n then chanceSpinDur=n end end })

    sec_chance:Toggle({ Title = "Anti Bait", Flag = "chanceAntiBait", Default = true,
        Callback = function(v) chanceAntiBait=v end, Type = "Checkbox"})

    sec_chance:Toggle({ Title = "Hold-to-Aim", Flag = "chanceHoldAim", Default = true,
        Callback = function(v) chanceHoldToAim=v end, Type = "Checkbox"})

    sec_chance:Dropdown({ Title = "Aim Key", Flag = "chanceAimKey",
        Values = {"Q","E","R","T","F","G","X","C","V"}, Value = "Q",
        Callback = function(v) chanceAimKey=Enum.KeyCode[v] end })

    sec_chance:Toggle({ Title = "Message When Aim", Flag = "chanceMsgOnAim", Default = false,
        Callback = function(v) chanceMsgOnAim=v end, Type = "Checkbox"})

    sec_chance:Input({ Title = "Message Text", Flag = "chanceMsgText", Placeholder = "...",
        Callback = function(v) chanceMsgText=v end })

    sec_chance:Button({
        Title = "Ping Aim Preset",
        Callback = function()
            chancePredMode = "Ping"; chancePredValue = 1.5
        end,
    })
...
local function g_attemptBlockForSound(sound, preId)
    if not guestCfg.soundBlockEnabled then return end
    ...
    -- Check if sound came from Gashing Wound gashing wound or Behead behead or Walkspeed Override - if yes, don't fire block
    if sound.Parent and sound.Parent.Name and (sound.Parent.Name:match("Gashing Wound") or sound.Parent.Name:match("gashing wound") or sound.Parent.Name:match("Behead") or sound.Parent.Name:match("behead") or sound.Parent.Name:match("Walkspeed Override") or sound.Parent.Name:match("walkspeed override") or sound.Parent.Name:match("WalkspeedOverride")) then
        return
    end
    ...
    if guestCfg.verifyFacingCheck then
        ...
        if dir.Magnitude > 0.01 then
            ...
            if distSq >= 25 and dot < cosA then return end
        end
    end
...
local function g_hookSound(sound)
    ...
    local pc = sound.Played:Connect(function()
        if guestCfg.soundBlockEnabled then
            local now = tick()
    ...
    local pp = sound:GetPropertyChangedSignal("IsPlaying"):Connect(function()
        if sound.IsPlaying and guestCfg.soundBlockEnabled then
            local now = tick()
            if not g_autoBlockLastCheck[preId] or (now - g_autoBlockLastCheck[preId]) > 0.05 then
                g_autoBlockLastCheck[preId] = now
                task.spawn(g_attemptBlockForSound, sound, preId)
    ...
    animBlockConn = svc.Run.Heartbeat:Connect(function()
        ...
        for _, killer in ipairs(kf:GetChildren()) do
            ...
            for _, track in ipairs(tracks) do
                ...
                -- Skip block for Gashing Wound gashing wound and Behead behead and Walkspeed Override animations
                if track.Animation and track.Animation.AnimationId then
                    local animIdStr = tostring(track.Animation.AnimationId)
                    if animIdStr:match("Gashing Wound") or animIdStr:match("gashing wound") or animIdStr:match("Behead") or animIdStr:match("behead") or animIdStr:match("Walkspeed Override") or animIdStr:match("walkspeed override") or animIdStr:match("WalkspeedOverride") then
                        continue
                    end
                end
...
-- ============================================================
-- NETWORK AUTO-BLOCK UI (NEW!)
-- ============================================================

secCore:Paragraph({ Title="━━━ Network Auto-Block ━━━", Content="Intercepts Slash, Punch, Stab at the network level" })

secCore:Toggle({
    Title = "🔗 Network Auto-Block",
    Description = "Hooks directly into the Network module for instant blocking",
    Type = "Checkbox",
    Flag = "guestNetworkBlock",
    Default = false,
    Callback = function(on)
        networkBlock_toggle(on)
    end
})

secCore:Slider({
    Title = "Network Block Cooldown (s)",
    Flag = "guestNetworkBlockCooldown",
    Step = 0.05,
    Value = { Min = 0.1, Max = 1.0, Default = networkBlock.blockCooldown },
    Callback = function(v)
        networkBlock.blockCooldown = v
    end
})

secCore:Toggle({
    Title = "Debug Mode",
    Description = "Print block events to console",
    Type = "Checkbox",
    Flag = "guestNetworkDebug",
    Default = false,
    Callback = function(v)
        networkBlock.debugMode = v
    end
})

local networkStatus = secCore:Paragraph({ Title = "Network Hook Status", Content = "Status: Waiting..." })

task.spawn(function()
    while true do
        task.wait(1)
        local status = networkBlock.hookActive and "🟢 ACTIVE" or "🔴 INACTIVE"
        local module = networkBlock.networkModule and "✅ Loaded" or "❌ Missing"
        local blocked = networkBlock.lastBlockTime > 0 and string.format("%.1fs ago", tick() - networkBlock.lastBlockTime) or "Never"
        networkStatus:Set({
            Title = "Network Hook Status",
            Content = "Hook: " .. status .. "\nModule: " .. module .. "\nLast Block: " .. blocked
        })
    end
end)
...
-- ============================================================
-- SOUND BLOCK (Existing)
-- ============================================================

secCore:Toggle({ 
    Title="Sound Block", 
    Type="Checkbox", 
    Flag="guestSoundBlockEnabled", 
    Default=false,
    Callback=function(v) 
        guestCfg.soundBlockEnabled = v
        guestCfg.autoBlockOn = v
        
        if v then
            print("[Guest Block] 🔊 Sound Block ENABLED - Listening for attack sounds...")
            pcall(function()
                local kf = getTeamFolder("Killers")
                if kf then
                    for _, killer in pairs(kf:GetChildren()) do
                        for _, d in pairs(killer:GetDescendants()) do
                            if d:IsA("Sound") then
                                if g.soundHooks[d] then
                                    for _, conn in ipairs(g.soundHooks[d]) do
                                        if type(conn) ~= "string" and conn and conn.Disconnect then
                                            pcall(function() conn:Disconnect() end)
                                        end
                                    end
                                    g.soundHooks[d] = nil
                                    g.soundBlockedUntil[d] = nil
                                end
                                pcall(g_hookSound, d)
                            end
                        end
                ...
                print("[Guest Block] ✅ Sound hooks refreshed")
            end)
        else
            print("[Guest Block] 🔇 Sound Block DISABLED")
            pcall(function()
...
secCore:Button({ 
    Title = "🔊 Test Sound Detection", 
    Callback = function()
        print("[Sound Block] 🔍 Scanning for sounds...")
        local count = 0
        local hooked = 0
        local kf = getTeamFolder("Killers")
        if kf then
            for _, killer in pairs(kf:GetChildren()) do
                for _, d in pairs(killer:GetDescendants()) do
                    if d:IsA("Sound") then
                        count = count + 1
                        local id = g_extractId(d)
                        local isTriggered = id and g_triggerSounds[id]
                        print("  Sound:", d.Name, "ID:", id or "unknown", 
                              "Triggered:", isTriggered and "✅ YES" or "❌ NO",
                              "Hooked:", g.soundHooks[d] and "✅" or "❌")
                        if g.soundHooks[d] then hooked = hooked + 1 end
                    end
                end
            end
        end
        print("[Sound Block] Found " .. count .. " sounds, " .. hooked .. " hooked")
        ui:Notify({
            Title = "Sound Scan Complete",
            Content = "Found " .. count .. " sounds, " .. hooked .. " hooked",
            Duration = 3
        })
    end 
})
...
local secTwoTime = tabTwoTime:Section({ Title = "Settings", Opened = true })

-- ── Config ──────────────────────────────────────────────────
local tt_DEFAULT_PROXIMITY   = 8
local tt_DEFAULT_DURATION    = 0.45
local tt_BEHIND_DISTANCE     = 3.5
local tt_CHECK_INTERVAL      = 0.05
local tt_COOLDOWN            = 5
local tt_LERP_SPEED          = 0.55
local tt_BEHIND_CONE_DEGREES = 70
local tt_REMOTE_FIRE_DELAY   = 0.0
local tt_AIM_SNAP_DELAY      = 0.25
local tt_DEBUG_LINE          = true

-- ── Mutable state ───────────────────────────────────────────
local tt_isRunning     = true
local tt_enabled       = false
local tt_daggerEnabled = false
local tt_rangeMode     = "Behind"
local tt_backstabType  = "Lerp"
local tt_proximity     = tt_DEFAULT_PROXIMITY
local tt_lastTrigger   = 0
local tt_aimRefCount   = 0
local tt_debugBeam     = nil

-- ── Debug line ──────────────────────────────────────────────
local function tt_removeDebugLine()
    if tt_debugBeam then
        pcall(function() tt_debugBeam:Destroy() end)
        tt_debugBeam = nil
    end
end

local function tt_drawDebugLine(hrp, khrp, isValid)
    if not tt_DEBUG_LINE then tt_removeDebugLine(); return end
    pcall(function()
        local att0 = hrp:FindFirstChild("__TTAtt0") or Instance.new("Attachment", hrp)
        att0.Name = "__TTAtt0"; att0.Position = Vector3.zero

        local att1 = khrp:FindFirstChild("__TTAtt1") or Instance.new("Attachment", khrp)
        att1.Name = "__TTAtt1"; att1.Position = Vector3.zero

        if not tt_debugBeam then
            local b = Instance.new("Beam")
            b.Name = "__TTBeam"; b.Attachment0 = att0; b.Attachment1 = att1
            b.FaceCamera = true; b.Width0 = 0.08; b.Width1 = 0.08
            b.Segments = 1; b.LightEmission = 1; b.LightInfluence = 0
            b.Parent = hrp
            tt_debugBeam = b
        end
        tt_debugBeam.Attachment0 = att0
        tt_debugBeam.Attachment1 = att1
        tt_debugBeam.Color = isValid
...
-- ── Helpers ─────────────────────────────────────────────────
local function tt_getCharacter()
    return lp.Character or lp.CharacterAdded:Wait()
end

local function tt_getDaggerButton()
...
local function tt_getDaggerCooldown()
    local btn = tt_getDaggerButton(); if not btn then return nil end
...
local function tt_readCooldownValue(cdObj)
...
local function tt_getKillersFolder()
    local p = svc.WS:FindFirstChild("Players")
    return p and p:FindFirstChild("Killers")
end

local function tt_isValidKillerModel(model)
...
local function tt_tryActivateButton(btn)
...
local function tt_setAutoRotate(value)
    local char = lp.Character; if not char then return end
...
-- ── Detection ────────────────────────────────────────────────
local function tt_isPlayerBehindKiller(hrp, khrp, dist)
    if dist > tt_proximity or dist < 0.01 then return false end
    local toPlayer  = (hrp.Position - khrp.Position).Unit
    local killerBack = -khrp.CFrame.LookVector
    local dot        = toPlayer:Dot(killerBack)
    local threshold  = math.cos(math.rad(tt_BEHIND_CONE_DEGREES))
    return dot >= threshold
end

-- ── Activate (lerp / teleport / aim) ────────────────────────
local function tt_activateForKiller(killerModel, duration)
    if not killerModel or not tt_isRunning then return end
    local char     = tt_getCharacter()
    ...
    tt_aimRefCount += 1
    if tt_aimRefCount == 1 then pcall(function() humanoid.AutoRotate = false end) end

    local function finishAiming()
        tt_aimRefCount = math.max(0, tt_aimRefCount - 1)
        if tt_aimRefCount == 0 then tt_setAutoRotate(true) end
    end

    local function computeBehindCFrame()
        local kCF      = khrp.CFrame
        local behindPos = kCF.Position - (kCF.LookVector.Unit * tt_BEHIND_DISTANCE)
        behindPos = Vector3.new(behindPos.X, kCF.Position.Y, behindPos.Z)
        return CFrame.new(behindPos, behindPos + kCF.LookVector.Unit)
    end

    if tt_backstabType == "Lerp" then
        local t0 = os.clock(); local conn
        conn = svc.Run.Heartbeat:Connect(function()
            if not tt_isRunning or os.clock() - t0 >= duration then
                conn:Disconnect(); finishAiming(); return
            end
            if khrp and hrp then
                hrp.CFrame = hrp.CFrame:Lerp(computeBehindCFrame(), tt_LERP_SPEED)
            end
        end)

    elseif tt_backstabType == "Teleport" then
        pcall(function() hrp.CFrame = computeBehindCFrame() end)
        task.delay(duration, finishAiming)

    elseif tt_backstabType == "Aim" then
        local t0 = os.clock(); local conn
        conn = svc.Run.Heartbeat:Connect(function()
            if not tt_isRunning or os.clock() - t0 >= duration then
                conn:Disconnect(); finishAiming(); return
            ...
            if khrp and hrp then
                ...
                hrp.CFrame = hrp.CFrame:Lerp(CFrame.new(hrp.Position, aimPos), tt_LERP_SPEED * 1.8)
            end
        end)
    end
end
...
-- ── Main watcher ─────────────────────────────────────────────
task.spawn(function()
    while tt_isRunning do
        task.wait(tt_CHECK_INTERVAL)
        if not tt_enabled or not tt_isRunning then continue end

        local killersFolder = tt_getKillersFolder()
        ...
        for _, killer in pairs(killersFolder:GetChildren()) do
            if triggered then break end
            if not tt_isValidKillerModel(killer) then continue end

            local khrp = killer:FindFirstChild("HumanoidRootPart")
            local dist = (khrp.Position - hrp.Position).Magnitude

            if dist <= tt_proximity then
                local valid = tt_isPlayerBehindKiller(hrp, khrp, dist)
                tt_drawDebugLine(hrp, khrp, valid)

                if valid
                and tt_rangeMode ~= "Around"
                and os.clock() - tt_lastTrigger >= tt_COOLDOWN then

                    local cdNum = tt_readCooldownValue(tt_getDaggerCooldown())
                    if not (cdNum and cdNum > 0.1) then
                        tt_lastTrigger = os.clock()
                        triggered = true

                        task.spawn(function()
                            tt_activateForKiller(killer, tt_DEFAULT_DURATION)

                            if tt_REMOTE_FIRE_DELAY > 0 then task.wait(tt_REMOTE_FIRE_DELAY) end
                            if tt_daggerEnabled then tt_tryActivateButton(tt_getDaggerButton()) end

                            if tt_AIM_SNAP_DELAY > 0 then
                                task.wait(tt_AIM_SNAP_DELAY)
                                if not tt_isRunning then return end
                                local khrp2 = killer:FindFirstChild("HumanoidRootPart")
                                local char2 = tt_getCharacter()
                                local hrp2  = char2 and char2:FindFirstChild("HumanoidRootPart")
                                if khrp2 and hrp2 then
                                    local behindPos = khrp2.CFrame.Position
                                        - (khrp2.CFrame.LookVector.Unit * tt_BEHIND_DISTANCE)
                                    behindPos = Vector3.new(behindPos.X, hrp2.Position.Y, behindPos.Z)
                                    hrp2.CFrame = CFrame.new(
                                        behindPos,
                                        behindPos + khrp2.CFrame.LookVector.Unit
                                    )
                                end
                            end
                        end)
            ...
            elseif tt_DEBUG_LINE then
                tt_removeDebugLine()
            end

            -- "Around" mode: proximity only, no angle check
            if tt_rangeMode == "Around"
            and dist <= tt_proximity
            and os.clock() - tt_lastTrigger >= tt_COOLDOWN
            and not triggered then
                local cdNum = tt_readCooldownValue(tt_getDaggerCooldown())
                if not (cdNum and cdNum > 0.1) then
                    tt_lastTrigger = os.clock()
                    triggered = true
        ...
        if not triggered then tt_removeDebugLine() end
    end
end)

-- ── UI Controls ──────────────────────────────────────────────
secTwoTime:Toggle({ Title = "Auto Backstab", Type = "Checkbox", Default = false,
    Callback = function(v) tt_enabled = v end })

secTwoTime:Toggle({ Title = "Auto Stab on Backstab", Type = "Checkbox", Default = false,
    Callback = function(v) tt_daggerEnabled = v end })

secTwoTime:Toggle({ Title = "Debug Line", Type = "Checkbox", Default = true,
    Callback = function(v)
        tt_DEBUG_LINE = v
        if not v then tt_removeDebugLine() end
    end })

secTwoTime:Dropdown({ Title = "Backstab Type", Values = {"Lerp", "Teleport", "Aim"}, Value = "Lerp",
    Callback = function(v) tt_backstabType = v end })

secTwoTime:Dropdown({ Title = "Range Mode", Values = {"Around", "Behind"}, Value = "Behind",
    Callback = function(v) tt_rangeMode = v end })

secTwoTime:Slider({ Title = "Detection Range", Step = 1,
    Value = { Min = 1, Max = 30, Default = tt_DEFAULT_PROXIMITY },
    Callback = function(v) tt_proximity = v end })

secTwoTime:Slider({ Title = "Behind Distance", Step = 0.5,
    Value = { Min = 0.5, Max = 10, Default = tt_BEHIND_DISTANCE },
    Callback = function(v) tt_BEHIND_DISTANCE = v end })

secTwoTime:Slider({ Title = "Behind Cone Angle (°)", Step = 5,
    Value = { Min = 10, Max = 180, Default = tt_BEHIND_CONE_DEGREES },
    Callback = function(v) tt_BEHIND_CONE_DEGREES = v end })

secTwoTime:Slider({ Title = "Remote Fire Delay (s)", Step = 0.01,
    Value = { Min = 0.00, Max = 0.50, Default = tt_REMOTE_FIRE_DELAY },
    Callback = function(v) tt_REMOTE_FIRE_DELAY = v end })

secTwoTime:Slider({ Title = "Aim Snap Delay (s)", Step = 0.01,
    Value = { Min = 0.00, Max = 0.30, Default = tt_AIM_SNAP_DELAY },
    Callback = function(v) tt_AIM_SNAP_DELAY = v end })

local secLegacyMS4 = tabTwoTime:Section({ Title = "Legacy Milestone IV TwoTime", Opened = true })

-- State for the skin script
local lm4_isRunning    = true
local lm4_hasUsedLife2 = false
local lm4_isProcessing = false
local lm4_active       = false
local lm4_connections  = {}

local function lm4_cleanup()
    for _, c in ipairs(lm4_connections) do pcall(function() c:Disconnect() end) end
    lm4_connections = {}
end

local function lm4_equipSkin()
    local remote = svc.RS:FindFirstChild("Modules")
        and svc.RS.Modules:FindFirstChild("Network")
        and svc.RS.Modules.Network:FindFirstChild("Network")
        and svc.RS.Modules.Network.Network:FindFirstChild("RemoteEvent")

    local skinAsset = svc.RS:FindFirstChild("Assets")
        and svc.RS.Assets:FindFirstChild("Skins")
        and svc.RS.Assets.Skins:FindFirstChild("Survivors")
        and svc.RS.Assets.Skins.Survivors:FindFirstChild("TwoTime")
        and svc.RS.Assets.Skins.Survivors.TwoTime:FindFirstChild("LegacyMS100TwoTime")
    ...
    if playerData then
        ...
        if purchased then
            ...
            if skins then
                ...
                for _, child in ipairs(skins:GetChildren()) do
                    if child.Name == "LegacyMS100TwoTime" then exists = true; break end
                end
                if not exists then
                    local v = Instance.new("IntValue")
                    v.Name = "LegacyMS100TwoTime"; v.Value = 1; v.Parent = skins
                end
            end
        end
        ...
        if equipped then
            ...
            if survivor then survivor.Value = "TwoTime" end
            local skins = equipped:FindFirstChild("Skins")
            if skins then
                local skinValue = skins:FindFirstChild("TwoTime")
                if skinValue then
                    skinValue.Value = "LegacyMS100TwoTime"
                else
                    local newSkin = Instance.new("StringValue")
                    newSkin.Name = "TwoTime"; newSkin.Value = "LegacyMS100TwoTime"
    ...
    if remote and skinAsset then
        local formats = {
            ...
            function() remote:FireServer("TwoTime", "LegacyMS100TwoTime") end,
        }
...
local function lm4_isDaggerOrWeapon(child)
    if not child then return false end
    local name = child.Name
    if name == "GhostFire" or name == "Dagger" or name == "DaggerTemplate" then return true end
    if name == "Crouch" or name == "CrouchStart" or name == "CrouchIdle" or
       name == "CrouchWalk" or name == "CrouchRun" or name == "CrouchStab" then return true end
    local wk = {"Dagger","Weapon","Sword","Blade","Knife","Tool","HopperBin","Handle","Grip",
        "Guard","Pommel","Tip","Edge","Point","Hitbox","Collision","Detector","Query",
        "Damage","Attack","Slash","Stab","Lunge","Fire","GhostFire"}
    for _, kw in ipairs(wk) do if string.find(name, kw) then return true end end
    if child:IsA("Tool") or child:IsA("HopperBin") then return true end
    if child:IsA("Model") then
        for _, d in ipairs(child:GetDescendants()) do
            if d:IsA("BasePart") then
                local pn = d.Name
                if string.find(pn,"Blade") or string.find(pn,"Handle") or string.find(pn,"Guard")
                or string.find(pn,"Dagger") or string.find(pn,"Crouch") then return true end
            end
        end
    end
    return false
...
local function lm4_shouldIgnorePart(partName)
    if not partName then return true end
    local ik = {"Hitbox","Collision","Detector","Query","Holder","HumanoidRootPart","Root",
        "Weapon","Blade","Handle","Guard","Pommel","Grip","Dagger","GhostFire","Lunge",
        "Stab","Damage","Attack","Crouch","CrouchStart","CrouchIdle","CrouchWalk",
        "CrouchRun","CrouchStab","Ritual","Respawn"}
    for _, kw in ipairs(ik) do if string.find(partName, kw) then return true end end
    return false
end

local function lm4_instantlyRemoveAccessories(character)
    if not character then return end
    local toRemove = {}
    for _, child in ipairs(character:GetChildren()) do
        if child.Name == "Head" or child.Name == "Humanoid" or child:IsA("Humanoid") then continue end
        if lm4_isDaggerOrWeapon(child) then continue end
    ...
    for _, item in ipairs(toRemove) do item:Destroy() end
end

local function lm4_runSkinInstant(forcedModelName)
    local character = lp.Character
    if not character then return false end
    -- Only apply skin if our survivor is TwoTime
    if character.Name ~= "TwoTime" then return false end
    local humanoid = character:FindFirstChildOfClass("Humanoid")
    if not humanoid or humanoid.Health <= 0 then return false end
    local survivors = svc.WS:FindFirstChild("Players") and svc.WS.Players:FindFirstChild("Survivors")
    if not survivors or not character:IsDescendantOf(survivors) then return false end

    local legacyMS4 = svc.RS:FindFirstChild("Assets")
    ...
    if not legacyMS4 then return true end

    local config = legacyMS4:FindFirstChild("Config")
    if not config then return true end
    local targetModel = config:FindFirstChild(forcedModelName)
    if not targetModel then return true end

    lm4_instantlyRemoveAccessories(character)

    local folder = Instance.new("Folder")
    folder.Name = "LegacyMS4_Attached"; folder.Parent = character
    ...
    if head then
        head.Transparency = 0; head.CanCollide = false
    ...
    for _, part in ipairs(targetModel:GetDescendants()) do
        ...
        if lm4_shouldIgnorePart(part.Name) then continue end

        local target = nil
        local pn = part.Name
        if pn == "hair" or pn == "halo" or pn == "hat" then target = head
        elseif pn == "Torso" or pn == "graphic" or pn == "belt" or pn == "scarf"
            or pn == "chest" or pn == "tail" or pn == "wings" or pn == "wing1" or pn == "wing2" then
            target = torso
        elseif string.find(pn,"Left") or string.find(pn,"left") then
            if string.find(pn,"Leg") then target = leftLeg
            elseif string.find(pn,"Arm") or string.find(pn,"Hand") or
                   string.find(pn,"Glove") or string.find(pn,"Bandage") then target = leftArm
            else target = torso end
        elseif string.find(pn,"Right") or string.find(pn,"right") then
            if string.find(pn,"Leg") then target = rightLeg
            elseif string.find(pn,"Arm") or string.find(pn,"Hand") or
                   string.find(pn,"Glove") or string.find(pn,"Bandage") then target = rightArm
            else target = torso end
        else target = torso end
        ...
        if target then
            ...
            newPart.Transparency = typeof(part.Transparency) == "number" and part.Transparency or 0
            newPart.Color = typeof(part.Color) == "Color3" and part.Color or Color3.new(1,1,1)
            newPart.Material = part.Material or Enum.Material.SmoothPlastic
            newPart.Anchored = false; newPart.CanCollide = false
            newPart.CanQuery = false; newPart.CanTouch = false; newPart.Massless = true

            local templateTarget = targetModel:FindFirstChild(target.Name, true)
            if templateTarget and templateTarget:IsA("BasePart") then
                local offset = templateTarget.CFrame:ToObjectSpace(part.CFrame)
                newPart.CFrame = target.CFrame * offset
            else
                newPart.CFrame = target.CFrame * modelPivot:ToObjectSpace(part.CFrame)
            end
            newPart.Parent = folder

            local weld = Instance.new("Weld")
            weld.Part0 = target; weld.Part1 = newPart
...
local function lm4_applySkin()
    if lm4_isProcessing then return end
    lm4_isProcessing = true
    local chosenModel = not lm4_hasUsedLife2 and "Life1" or "Life2"
    local success = lm4_runSkinInstant(chosenModel)
    local attempts = 0
    while not success and attempts < 5 do
        task.wait(0.1); success = lm4_runSkinInstant(chosenModel); attempts += 1
    end
    lm4_isProcessing = false
end

local function lm4_changeTwoTimeIcon()
    local pg = lp:FindFirstChild("PlayerGui"); if not pg then return end
    local tempUI = pg:FindFirstChild("TemporaryUI"); if not tempUI then return end
    local playerInfo = tempUI:FindFirstChild("PlayerInfo"); if not playerInfo then return end
    local currentSurvivors = playerInfo:FindFirstChild("CurrentSurvivors"); if not currentSurvivors then return end
    for _, frame in ipairs(currentSurvivors:GetChildren()) do
        if frame:IsA("Frame") then
            local username = frame:FindFirstChild("Username")
            if username and username:IsA("TextLabel") and string.find(username.Text, lp.Name) then
...
local function lm4_findRespawnPart()
    local map = svc.WS:FindFirstChild("Map"); if not map then return nil end
    local ingame = map:FindFirstChild("Ingame"); if not ingame then return nil end
    return ingame:FindFirstChild(lp.Name .. "RespawnLocation")
end

local function lm4_isAtRespawn(character, respawnPart)
    ...
    local charPos = hrp.Position; local respawnPos = respawnPart.Position
    local dist = (charPos - respawnPos).Magnitude
    local isAbove = math.abs(charPos.X - respawnPos.X) < 5 and math.abs(charPos.Z - respawnPos.Z) < 5
    return dist < 15 or isAbove
end

local function lm4_start()
    if lm4_active then return end
    lm4_active = true
    lm4_hasUsedLife2 = false
    lm4_cleanup()

    task.spawn(lm4_equipSkin)

    -- Reset Life state when a new round starts (Ingame folder appears)
    local function lm4_watchForNewRound()
        local map = svc.WS:FindFirstChild("Map")
        local function watchMap(m)
            if not m then return end
            table.insert(lm4_connections, m.ChildAdded:Connect(function(child)
                if child.Name == "Ingame" then
                    lm4_hasUsedLife2 = false
                    print("[TwoTime] New round detected - reset to Life1")
                end
            end))
        end
        watchMap(map)
        table.insert(lm4_connections, svc.WS.ChildAdded:Connect(function(child)
            if child.Name == "Map" then
                task.wait(0.1)
                watchMap(child)
            end
    ...
    lm4_watchForNewRound()

    local respawnPart = lm4_findRespawnPart()

    local survivors = svc.WS:FindFirstChild("Players") and svc.WS.Players:FindFirstChild("Survivors")
    if survivors then
        table.insert(lm4_connections, survivors.ChildRemoved:Connect(function(child)
            if child == lp.Character then lm4_hasUsedLife2 = false end
        end))
        table.insert(lm4_connections, survivors.ChildAdded:Connect(function(child)
            if child:IsA("Model") then
                for _, plr in ipairs(svc.Players:GetPlayers()) do
                    if plr.Character == child and plr == lp then
                        lm4_hasUsedLife2 = false; break
                    end
                end
            end
        end))
    ...
    table.insert(lm4_connections, lp.CharacterAdded:Connect(function(char)
        task.wait(0.1); lm4_instantlyRemoveAccessories(char)
    end))

    table.insert(lm4_connections, lp.CharacterAdded:Connect(function(char)
        task.wait(0.5)
        local character = lp.Character; if not character then return end
        if respawnPart and lm4_isAtRespawn(character, respawnPart) then
            if lm4_hasUsedLife2 then
                lm4_applySkin(); lm4_changeTwoTimeIcon()
            else
                lm4_hasUsedLife2 = true; lm4_applySkin()
            end
        else
            if not lm4_hasUsedLife2 then
                lm4_hasUsedLife2 = true; lm4_applySkin()
            end
        end
    end))
    ...
    local pg = lp:FindFirstChild("PlayerGui")
    if pg then
        table.insert(lm4_connections, pg.DescendantAdded:Connect(function(desc)
            if desc.Name == "TemporaryUI" and desc:IsA("ScreenGui") then
                task.wait(0.5); lm4_changeTwoTimeIcon()
            end
        end))
    end

    -- Apply immediately if already in a match
    if lp.Character then
        task.spawn(function()
            task.wait(0.5); lm4_applySkin()
        end)
    end

    print("[TwoTime] Legacy Milestone IV active")
end

local function lm4_stop()
    lm4_active = false
    lm4_cleanup()
    print("[TwoTime] Legacy Milestone IV stopped")
end

secLegacyMS4:Button({ Title = "Legacy Milestone IV TwoTime",
    Callback = function()
        if lm4_active then
            lm4_stop()
        else
...
secTwoTime:Button({ Title = "Unload TwoTime",
    Callback = function()
        tt_isRunning = false
        tt_enabled   = false
        tt_setAutoRotate(true)
        tt_removeDebugLine()
        lm4_stop()
        tabTwoTime:Destroy()
    end })
...
function applyConfigFromFlags()
    pcall(function()
        ...
        if win.Flags.guestSoundBlockEnabled ~= nil then 
            guestCfg.soundBlockEnabled=win.Flags.guestSoundBlockEnabled
            if guestCfg.soundBlockEnabled then
        ...
        if win.Flags.guestHitboxEnabled ~= nil then guestCfg.hitboxEnabled=win.Flags.guestHitboxEnabled; if guestCfg.hitboxEnabled then setupHitboxBlock() end end
        if win.Flags.guestAnimEnabled ~= nil then guestCfg.animEnabled=win.Flags.guestAnimEnabled; if guestCfg.animEnabled then setupAnimBlock() end end
        if type(win.Flags.guestHitboxX)=="number" then guestCfg.hitboxSize=Vector3.new(win.Flags.guestHitboxX, guestCfg.hitboxSize.Y, guestCfg.hitboxSize.Z) end
        if type(win.Flags.guestHitboxY)=="number" then guestCfg.hitboxSize=Vector3.new(guestCfg.hitboxSize.X, win.Flags.guestHitboxY, guestCfg.hitboxSize.Z) end
        if type(win.Flags.guestHitboxZ)=="number" then guestCfg.hitboxSize=Vector3.new(guestCfg.hitboxSize.X, guestCfg.hitboxSize.Y, win.Flags.guestHitboxZ) end
        if type(win.Flags.guestAutoRange)=="number" then guestCfg.detectionRange=win.Flags.guestAutoRange end
        if type(win.Flags.guestAutoDelay)=="number" then guestCfg.blockDelay=win.Flags.guestAutoDelay end
        if win.Flags.guestAutoBlockType ~= nil then guestCfg.blockType=win.Flags.guestAutoBlockType end
        if win.Flags.guestAutoDouble ~= nil then guestCfg.doubleBlock=win.Flags.guestAutoDouble end
        ...
        if win.Flags.ttEnabled ~= nil then tt_enabled=win.Flags.ttEnabled end
        if win.Flags.ttDaggerEnabled ~= nil then tt_daggerEnabled=win.Flags.ttDaggerEnabled end
        if win.Flags.ttBackstabType ~= nil then tt_backstabType=win.Flags.ttBackstabType end
        if win.Flags.ttRangeMode ~= nil then tt_rangeMode=win.Flags.ttRangeMode end
        
        -- Network block flags
        if win.Flags.guestNetworkBlock ~= nil then
            networkBlock_toggle(win.Flags.guestNetworkBlock)
        end
        if type(win.Flags.guestNetworkBlockCooldown) == "number" then
            networkBlock.blockCooldown = win.Flags.guestNetworkBlockCooldown
        end
        if win.Flags.guestNetworkDebug ~= nil then
            networkBlock.debugMode = win.Flags.guestNetworkDebug
        end
