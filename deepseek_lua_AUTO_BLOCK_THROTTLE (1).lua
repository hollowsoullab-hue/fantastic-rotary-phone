-- BetrayalHub - Complete Final Version (FIXED AIMBOT)
...
-- ============================================================
-- AIMBOT TAB (FIXED - Using __namecall hook)
-- ============================================================
...
-- THROTTLE AUTO BLOCK - Throttle per SOUND ID, not per sound object
-- This allows multiple instances of the same attack to get blocked
...
local function g_hookSound(sound)
    ...
    local pc = sound.Played:Connect(function()
        if guestCfg.autoBlockOn then
            -- THROTTLE PER ID: Only check every 0.05 seconds per sound ID (same attacks still block)
            local now = tick()
    ...
    local pp = sound:GetPropertyChangedSignal("IsPlaying"):Connect(function()
        if sound.IsPlaying and guestCfg.autoBlockOn then
            -- THROTTLE PER ID: Only check every 0.05 seconds per sound ID
            local now = tick()
            if not g_autoBlockLastCheck[preId] or (now - g_autoBlockLastCheck[preId]) > 0.05 then
                g_autoBlockLastCheck[preId] = now
                task.spawn(g_attemptBlockForSound, sound, preId)
